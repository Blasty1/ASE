...
extern volatile status ModalitaSistema;
extern volatile int8_t valore_iniziale;
extern volatile int32_t tempi_variazioni[MAX];
extern volatile int8_t counterVar;
...
{			
	...
	if(down_2 !=0) {			/* KEY2 */
		...
		if((LPC_GPIO2->FIOPIN & (1<<12)) == 0){ /* button premuto */
			switch(down_2){
				case 2:
					ModalitaSistema=Acquisizione;
					counterVar=0;
					LED_Out(0xFF); //led tutti accesi
					init_timer(3,0,SRI,25000000);
					disable_timer(0);
					enable_timer(3);
	...
	/* Joystick UP */
	if(ModalitaSistema == Acquisizione)
	{
		...
		if(J_up == 0 || J_up == 1){
			if(valore_iniziale != J_up)
			{
				tempi_variazioni[counterVar++]=J_up; //metto quello finale 
				valore_iniziale = J_up;
			}
			if(counterVar == MAX + 1)
			{
